...
def addJsonWhere(path):
    with open(path, 'r', encoding='utf8') as fp:
        json_data = json.load(fp)

    JDBCURL = json_data['job']['content'][0]['reader']['parameter']['connection'][0]['jdbcUrl'][0]
    TABLE_NAME = json_data['job']['content'][0]['reader']['parameter']['connection'][0]['table'][0]
    dt,start_time,end_time = generateJsonWhere(JDBCURL,TABLE_NAME)
    WHERE = dt + ' between \'' + start_time + '\' and \'' + end_time + '\''
    json_data['job']['content'][0]['reader']['parameter']['where'] = WHERE
    with open(path, 'w', encoding='utf8') as fp1:
        fp1.write(json.dumps(json_data, ensure_ascii=False, indent=4))
    fp.close()
    fp1.close()

def generateJsonWhere(JDBCURL,TABLE_NAME):
    conn = psycopg2.connect(host='192.168.100.25',
                            port=5432,
                            database='postgres',
                            user='postgres',
    ...
    END_TIME = results[0][2]
    cur.execute(updateSql(JDBCURL,TABLE_NAME))
    cur.execute(insertSql(JDBCURL,TABLE_NAME,DT,START_TIME,END_TIME))
    conn.commit()
    cur.close()
    conn.close()
    return DT,START_TIME,END_TIME

def updateSql(jdbcURL,table_name):
    sql = '''
    update test.datax_etl_log 
    set new_flag = 0
    where jdbc = '%s'
    and table_name = '%s'
    and new_flag = 1;
...
def insertSql(jdbcURL,table_name,insert_data,start_time,end_time):
    sql = '''
    insert into test.datax_etl_log(
        jdbc,
        table_name,
        increment_dt,
        start_time,
    ...
    '%s',
    '%s','%s','%s',now(),1);
    '''%(jdbcURL,table_name,insert_data,start_time,end_time)
    return sql

def selectSql(jdbcurl, table_name):
    sql = '''
    select 
        increment_dt,
        to_char(end_time,'YYYY-MM-DD hh24:mi:ss') as start_time, 
        to_char(now(),'YYYY-MM-DD hh24:mi:ss') as end_time
    from test.datax_etl_log 
    where jdbc = '%s'
    and table_name = '%s'
    and new_flag = 1
    '''%(jdbcurl, table_name)
